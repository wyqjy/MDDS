...
def get_ms_train_transform():
    print("Building transform_train")
    tfm_train = []

    interp_mode = INTERPOLATION_MODES['bilinear']
    tfm_train += [Resize(224, interpolation=interp_mode)]
    tfm_train += [Random2DTranslation(224, 224)]
    tfm_train += [RandomHorizontalFlip()]
    tfm_train += [ToTensor()]
...
def get_ms_test_transform():
    print("Building transform_test")
    tfm_test = []

    interp_mode = INTERPOLATION_MODES['bilinear']
    tfm_test += [Resize(224, interpolation=interp_mode)]
    tfm_test += [CenterCrop(224)]
    tfm_test += [ToTensor()]
